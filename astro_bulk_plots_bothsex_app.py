    ...
    for row_genes in rows:
        ...
        for i, search_gene in enumerate(row_genes):
            with cols[i]:
                ...
                search_summary["GenotypeShort"] = search_summary["Genotype"].astype(str).apply(
                    lambda x: "Aldh1l1" if "Aldh1l1" in x else "Gfap"
                )
                search_summary["LineGroup"] = (
                    search_summary["Treatment"].astype(str) + " | " +
                    search_summary["CellType"].astype(str) + " | " +
                    search_summary["SexGroup"].astype(str) + " | " +
                    search_summary["GenotypeShort"].astype(str)
                )
                ...
                # 🔍 Only keep "Both" sex-pooled entries
                search_summary_both = search_summary[search_summary["SexGroup"] == "Both"]
                ...
                fig = px.line(
                    ...
                    line_group="CellType",
                    markers=True,
                    title=f"Mean ± SD Expression of {search_gene} (Sex-pooled)",
                ...
                # Abbreviate genotype name
                search_summary_both["GenotypeShort"] = search_summary_both["Genotype"].astype(str).apply(
                    lambda x: "Aldh1l1" if "Aldh1l1" in x else "Gfap"
                )

                # Force conversion of Time to string explicitly
                search_summary_both["TimeStr"] = search_summary_both["Time"].astype(str)

                # Apply tick labels using Label
                tickvals = search_summary_both["Group"].tolist()
                ticktext = search_summary_both["TimeStr"].tolist()
